...
const Menu = () => {
  ...
  const handleAddCartProducts = (e) => {
    if (!isAuthenticated) {
      toast.error('Login first then Save it later');
      return;
    }
    dispatch(addCartItem(productDisplay))
    toast.success(`${productDisplay.name} saved to later`);
  };
      ...
      <div className='w-full max-w-4xl bg-white mx-auto md:flex rounded shadow-lg'>
        ...
        <div className='w-full md:w-1/2 p-4'>
          ...
          {isAuthenticated && (
            <>
              <div>
                <h6 className='font-semibold text-slate-600 capitalize text-lg mb-2'>Basic Info</h6>
                <p className='py-2 text-base'>{productDisplay.ingridents}</p>
              </div>
              <div>
                <h6 className='font-semibold text-slate-600 capitalize text-lg mb-2'>Detailed Information</h6>
                <p className='py-2 text-base'>{productDisplay.recipe}</p>
              </div>
            </>
          )}
          <div className='flex justify-center mt-4'>
            <button onClick={handleAddCartProducts} className='bg-yellow-400 hover:bg-yellow-500 py-2 px-4 rounded-full text-black text-xl font-medium'>Save it Later</button>
          </div>
        </div>
      </div>
